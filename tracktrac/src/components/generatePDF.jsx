...
const GeneratePdf = ({ data }) => {
    const generatePDF = () => {
        ...
        if (!data || !Array.isArray(data.yearlyData) || !Array.isArray(data.topSongs) || !Array.isArray(data.topArtists) || !data.structuredMonthlyData) {
        ...
        // Loop over structuredMonthlyData for each year
        Object.keys(data.structuredMonthlyData).forEach((year) => {
            // Title for the year
            if (currentY > 270) {
            ...
            doc.setFontSize(12);
            doc.text(`Year: ${year}`, 20, currentY);
            currentY += 10;
    
            // For each month in the year
            const monthlyData = data.structuredMonthlyData[year];
            Object.entries(monthlyData).forEach(([month, count]) => {
              if (currentY > 270) { // Comprobar si el mes cabe en la página actual
                doc.addPage();  // Si no cabe, agregar una nueva página
                currentY = 20;  // Restablecer la posición Y en la nueva página
              }
              doc.text(`${month}: ${count} streams`, 20, currentY);
              currentY += 6; // Reducir el espaciado para cada mes
            });
    
            // Add a space after each year
            currentY += 10;
          });
